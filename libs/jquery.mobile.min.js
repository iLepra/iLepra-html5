/*! jQuery Mobile vGit Build: SHA1: 896678e370941e69e7315f4a7f003bdf5375abe6 <> Date: Tue Feb 28 17:52:29 2012 +0700 jquerymobile.com | jquery.org/license */
...
this.list.appendTo(this.listbox),this._focusButton()),this.isOpen=false},open:function(){function b(){c.list.find("."+a.mobile.activeBtnClass).focus()}if(!this.options.disabled){var c=this,d=a(q),e=c.list.parent(),f=e.outerHeight(),e=e.outerWidth();a(".ui-page-active");var g=d.scrollTop(),h=c.button.offset().top,j=d.height(),d=d.width();c.button.addClass(a.mobile.activeBtnClass);setTimeout(function(){c.button.removeClass(a.mobile.activeBtnClass)},300);if(f>j-80||!a.support.scrollTop){c.menuPage.appendTo(a.mobile.pageContainer).page();
...
q.setAttribute("tabindex","-1");q.appendChild(v);m.appendChild(q)}m.firstChild.setAttribute("tabindex",0);this.list[0].appendChild(m);this.isMultiple||this.headerClose.hide();!this.isMultiple&&!c.length?this.header.hide():this.headerTitle.text(this.placeholder);this.list.listview()},_button:function(){return a("<a>",{href:"#",role:"button",id:this.buttonId,"aria-haspopup":"true","aria-owns":this.menuId})}})};a(k).delegate("select","selectmenubeforecreate",function(){var b=a(this).data("selectmenu");
b.options.nativeMenu||c(b)})})(jQuery);(function(a){a.widget("mobile.fixedtoolbar",a.mobile.widget,{options:{visibleOnPageShow:true,disablePageZoom:true,transition:"slide",fullscreen:false,tapToggle:true,tapToggleBlacklist:"a, input, select, textarea, .ui-header-fixed, .ui-footer-fixed",hideDuringFocus:"input, textarea, select",updatePagePadding:true,trackPersistentToolbars:true,supportBlacklist:function(){var a=q,b=navigator.userAgent,g=navigator.platform,e=b.match(/AppleWebKit\/([0-9]+)/),e=!!e&&
e[1],d=b.match(/Fennec\/([0-9]+)/),d=!!d&&d[1],f=b.match(/Opera Mobile\/([0-9]+)/),h=a.blackberry&&a.navigator.appVersion.match(/Version\/([0-9]+)/),h=!!h&&parseInt(h[1],10);return(g.indexOf("iPhone")>-1||g.indexOf("iPad")>-1||g.indexOf("iPod")>-1)&&e&&e<534||a.operamini&&{}.toString.call(a.operamini)==="[object OperaMini]"||f&&omverson<7458||b.indexOf("Android")>-1&&e&&e<533||d&&d<6||"palmGetResource"in q&&e&&e<534||a.blackberry&&h<7||b.indexOf("MeeGo")>-1&&b.indexOf("NokiaBrowser/8.5.0")>-1?true:
false},initSelector:":jqmData(position='fixed')"},_create:function(){var a=this.options,b=this.element,g=b.is(".ui-header")?"header":"footer",e=b.closest(".ui-page");a.supportBlacklist()?this.destroy():(b.addClass("ui-"+g+"-fixed"),b.jqmData("fullscreen")?(b.addClass("ui-"+g+"-fullscreen"),e.addClass("ui-page-"+g+"-fullscreen")):e.addClass("ui-page-"+g+"-fixed"),this._addTransitionClass(),this._bindPageEvents(),this._bindToggleHandlers())},_addTransitionClass:function(){var a=this.options.transition;
a&&a!=="none"&&(a==="slide"&&(a=this.element.is(".ui-header")?"slidedown":"slideup"),this.element.addClass(a))},_bindPageEvents:function(){var c=this,b=c.options;c.element.closest(".ui-page").bind("pagebeforeshow",function(){b.disablePageZoom&&a.mobile.zoom.disable(true);b.visibleOnPageShow&&c.show(true)}).bind("webkitAnimationStart animationstart updatelayout",function(){b.updatePagePadding&&c.updatePagePadding()}).bind("pageshow",function(){c.updatePagePadding();b.updatePagePadding&&a(q).bind("throttledresize."+
c.widgetName,function(){c.updatePagePadding()})}).bind("pagebeforehide",function(g,e){b.disablePageZoom&&a.mobile.zoom.enable(true);b.updatePagePadding&&a(q).unbind("throttledresize."+c.widgetName);if(b.trackPersistentToolbars){var d=a(".ui-footer-fixed:jqmData(id)",this),f=a(".ui-header-fixed:jqmData(id)",this),h=d.length&&e.nextPage&&a(".ui-footer-fixed:jqmData(id='"+d.jqmData("id")+"')",e.nextPage),j=f.length&&e.nextPage&&a(".ui-header-fixed:jqmData(id='"+f.jqmData("id")+"')",e.nextPage);if(h.length||
j.length)h.add(j).appendTo(a.mobile.pageContainer),e.nextPage.one("pageshow",function(){h.add(j).appendTo(this)})}})},_visible:false,updatePagePadding:function(){var a=this.element,b=a.is(".ui-header");this.options.fullscreen||a.closest(".ui-page").css("padding-"+(b?"top":"bottom"),a.outerHeight())},show:function(c){var b=this.element,g=a(q),e=g.scrollTop(),d=b.height(),f=b.closest(".ui-page").height(),g=Math.min(screen.height,g.height()),h=b.is(".ui-header")?"header":"footer";!c&&(this.options.transition&&
this.options.transition!=="none"&&(h==="header"&&!this.options.fullscreen&&e>d||h==="footer"&&!this.options.fullscreen&&e+g<f-d)||this.options.fullscreen)?b.removeClass("out ui-fixed-hidden").addClass("in"):b.removeClass("ui-fixed-hidden");this._visible=true},hide:function(c){var b=this.element,g=a(q),e=g.scrollTop(),d=b.height(),f=b.closest(".ui-page").height(),g=Math.min(screen.height,g.height()),h=b.is(".ui-header")?"header":"footer",j="out"+(this.options.transition==="slide"?" reverse":"");!c&&
